...
#$testIsparent |? {$_.id -eq  "ir2258175"}  | select * | Out-GridView | NO TIENE HIJOS

$crearIncidentesHijos = @()

$testIsparent |? {$_.id -eq  "ir1612296"} | ForEach-Object {

$_.id
$hijitos = Get-hijosIncidentes $_ $servidorOrigen
$hijitos
$crearIncidentesHijos += $hijitos
write-host "-------------------------------------------------------------------------------"

Get-SCSMObject -Class $IRClass -Filter "ID -eq $IR”  -ComputerName $servidorDestino  | Remove-SCSMObject -Force -ComputerName $servidorDestino 
...
#$crearIncidentesHijos |? {$_.id -eq  "ir2128217"} | ForEach-Object { |? {$_.id -eq  "ir1612296"}
Measure-Command{
...
$contadormax = $testIsparent.Count
...
#2147010,ir2128217 IR2145681 <----- error al importar al action log, psoiblemente el comentario es extenso y con muchos espacios

$testIsparent | select -first $lote | ForEach-Object {

$testIsparent |? {$_.id -eq  "IR2147010"}| ForEach-Object {
...
write-host "proximos 10" -ForegroundColor Cyan

$testIsparent | select -first $lote  -Skip $lote | ForEach-Object {
...
$newReq = New-SCSMObject -Class $IRclassDestino -PropertyHashtable $properties -PassThru -ComputerName $servidorDestino #-Credential $cred
 ...
 -join($wi.id, $exception  ) | out-file $logPath -Append
...
}until($contador -eq $contadormax) #fin de DO


Write-Host "---------------- Creacion de incidentes finalizado --------------------------"
